...
CleanExit:
    ...
    If gChanges.Count = 0 Then
        MsgBox "Nebyly provedeny žádné zmìny. Skryté, nevybrané a zamèené listy byly pøeskoèeny.", vbInformation, TOOL_TITLE
    Else
        frmReview.Show
    End If
    Exit Sub
...
Public Sub UndoChange(ByVal index As Long)
    ...
    If gTargetWorkbook.Worksheets(record.SheetName).ProtectContents Then
        MsgBox "List je nyní zamèený. Zmìnu nelze zrušit.", vbExclamation, TOOL_TITLE
        Exit Sub
    End If
